...
public class BezierCurve
{
    ...
    public Vector3[] GetSegments(int Subdivisions)
    {
        ...
        if (lastSegments == null) lastSegments = new Vector3[segments.Length];

        float time;
        for (int i = 0; i < Subdivisions; i++)
        {
            time = (float)i / Subdivisions;
            segments[i] = Vector3.Lerp(lastSegments[i], GetSegment(time),  8 * Time.deltaTime);
        }
